...
//    console.log(`d: ${d}`)
// errors at 3.045..e-13 tyger glyph
// but should we check for this
    if (d < EPS) {
      console.log("linesIntersect 1");
      return true
    }
  } else {
...
},{"buffer":9,"fs":8}],6:[function(require,module,exports){
...
// calculate bezezier resolution
// if resolution to high, CAG Intersection errors occur in path.toInnerCAG
// res * resFactor
// p1 can be point and p2 can be controle point...
// assuming that control points are same dist?
const calcBezRes = (res, p1, p2) => {
  console.log("calcBezRes");
  console.log(p1);
  console.log(p2);
  ...
  let ya = p1.y - p2.y;
  // sqrt not needed.. 
  let l = Math.pow(xa,2) + Math.pow(ya,2);
  console.log(`res: ${res}, xa: ${xa}, ya: ${ya} l: ${l}`);

  // 10 seems to be nice value
  if (l < 0) {
    // make new res, should it be an int?
    // smaller needs smaller res
    //return Math.round(res * (l/10)); // 1/32 10/32
    return 0;//Math.round(res * (l/10)); // 1/32 10/32
  } else {
    return res;
  } 
}
...
//        console.log("got close");
        ...
        np = np.close();
        csgPath.push(np);
//        if (csgPath.length == 4 || csgPath.length == 5) {
//          console.log("lucifer");
//        }
        break;
      case "L": // line
...
//        if (cmd.x1 == cmd.x2) { console.log("break x1=x2"); break }; // specific for hands?
 //       if (cmd.y1 == cmd.y2) { console.log("break y1=y2"); break }; // specific for hands?
        ...
        // TODO make resolution dynamic? resolution down if points to close together
        // should that be done in appenBezier Function?
        prevPoint = np.points[np.points.length-1];
  ...
  if (cp.length > 0) {
    ...
    const cags = cp.map(p => {
      try {
        ...
        let itc = fromPointsArray(p.points);
        i++;
        return itc;
      } catch (e) {
        // intersecting?
...
/*
  check if half? of a subset? of points are
  in the otherCag
*/
const fastCagInCag = (cagA,cagB) => {
//  let p = cag.sides[0].vertex0.pos;
  let l = cagA.sides.length;
  let icnt = 0;
  // TODO if l is smaller than 4?
  // take the center of bounding box? 

  // test max 8? points
  let step = Math.ceil(l/8); // aargh round will test same point twice
  //let step = 2;
  let p;
  for (let i=0; i < l; i+=step) {
    p = cagA.sides[i].vertex0.pos;
//    console.log(`i:${i} p:${JSON.stringify(p)}`);
    if (pointInCag(cagB,p)) { 
      icnt++;
     // console.log(`pic: ${icnt}`);
    }  
  }  
  let t = Math.round(l/step);
//  console.log(`l: ${l}`);
//  console.log(`fastCagInCag: ${icnt}/${t}`);

  // 80 % difficult if 2 points tested... 
  return ((icnt/t) > 0.49);
}
...
//    console.log("fitInTree:");
    ...
    for (el of branch) {
      ...
      let p2 = el.cag.sides[0].vertex0.pos;
//      console.log(p2);
      fastCagInCag(cag, el.cag);

      //if(cagInCag(cag, el.cag)) {
      if(pointInCag(cag, p2)) {
      ...
      if(fastCagInCag(cag, el.cag)) { // TODO swap arguments????
...
// copied and modified.. EPS in isSelfIntersectioni (lineUtils) is to big!?
const fromPointsArray = function (points) {
  if (points.length < 3) {
    throw new Error('CAG shape needs at least 3 points')
  }
  let sides = []
  let prevvertex = new CAG.Vertex(new CSG.Vector2D(points[points.length - 1]))
  points.map(function (point) {
    let vertex = new CAG.Vertex(new CSG.Vector2D(point))
...
},{"@jscad/csg/src/core/utils/cagValidation":4,"opentype.js":5}],7:[function(require,module,exports){
'use strict'
